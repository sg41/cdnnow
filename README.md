...
Методика: один и тот же генератор (`bin/generator`, 20 потоков,
`--interval 0`), по ~12 с нагрузки на каждый сервер, 0 ошибок в обоих
прогонах. Обе версии использовали одни и те же нативные `.so`.

| Сценарий | RPS |
|---|---|
| Python `calculator_server.py` + Go-генератор (20 потоков) | ~3 100 (37 223 req / 12.06 с) |
| Go `calculator_server` + Go-генератор (20 потоков, interval 0) | ~96 300 (1 157 223 req / 12.02 с; по полным секундам `/metrics`: 94–99k) |

Ускорение **~31x** (96 305 / 3 085). p95/p99 под нагрузкой: C ~22/~54 мкс,
Rust ~19/~50 мкс (плавают от частоты CPU). Инвариант `sum+sub==0` сошёлся
(`-49053/49053` после ~1.16M конкурентных запросов).
